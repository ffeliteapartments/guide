...
function openSettings() {
  ...
  // Se Firebase Auth è abilitato, mostra il form Firebase invece del PIN
  if (typeof isFirebaseAuthEnabled === 'function' && isFirebaseAuthEnabled()) {
    const fbView = document.getElementById('firebase-login-view');
    if (fbView) {
      document.getElementById('pin-view').style.display = 'none';
      document.getElementById('login-view').style.display = 'none';
      fbView.style.display = 'block';
      const fbEmailEl = document.getElementById('fb-login-email');
      const fbErrEl   = document.getElementById('fb-login-error');
      if (fbEmailEl) fbEmailEl.value = '';
      const fbPassEl = document.getElementById('fb-login-password');
      if (fbPassEl) fbPassEl.value = '';
      if (fbErrEl) fbErrEl.textContent = '';
...
function init() {
  ...
  // Se Firebase è configurato, carica la guida da Firestore in background
  // (aggiorna currentData e ri-renderizza se ci sono dati più recenti)
  if (typeof isFirebaseConfigured === 'function' && isFirebaseConfigured() &&
      typeof loadGuideFromFirestore === 'function') {
    var slug = typeof getGuideSlugFromUrl === 'function' ? getGuideSlugFromUrl() : 'default';
    loadGuideFromFirestore(slug);
  }
}
...
function initEventListeners() {
  ...
  // ── Firebase Auth form ─────────────────────
  var fbSubmitBtn = document.querySelector('.fb-login-submit-btn');
  if (fbSubmitBtn) fbSubmitBtn.addEventListener('click', function() {
    if (typeof submitFirebaseLogin === 'function') submitFirebaseLogin();
  });
  var fbCancelBtn = document.querySelector('.fb-login-cancel-btn');
  if (fbCancelBtn) fbCancelBtn.addEventListener('click', closePinModal);
